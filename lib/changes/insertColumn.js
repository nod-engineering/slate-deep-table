...
function insertColumn(opts, editor, at) {
    ...
    table.nodes.forEach((rowNode, index) => {
        ...
        if (isMerged) {
            if (mergeDirection.right && rowNode.nodes.find(rowCell => rowCell.key === mergeCentre)) {
            ...
            if (mergeDirection.down || mergeDirection.right) {
                if (mergeDirection.right && !mergeDirection.down && rowNode.nodes.find(rowCell => rowCell.key === mergeCentre)) {
                    addMergedCell = true;
                } else if (mergeDirection.right && mergeDirection.down && !column.find(cell => cell.key === mergeCentre)) {
                    addMergedCell = true;
                }
            }
